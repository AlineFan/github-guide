...
// Detect mode: github-guide (uses <section>) vs frontend-slides (uses .slide)
const mode = await page.evaluate(() => {
  if (document.querySelectorAll('.slide').length > 0) return 'slides';
  if (document.querySelectorAll('main > section, body > section').length > 0) return 'sections';
  return 'unknown';
});

if (mode === 'unknown') {
  console.error('  ERROR: No .slide or <section> elements found.');
  console.error('  This script supports two formats:');
  console.error('    1. frontend-slides: <div class="slide"> or <section class="slide">');
  console.error('    2. github-guide:    <main><section id="..."> with scroll-snap');
  await browser.close();
  server.close();
  process.exit(1);
}

const SELECTOR = mode === 'slides' ? '.slide' : 'main > section, body > section';

const slideCount = await page.evaluate((sel) => document.querySelectorAll(sel).length, SELECTOR);

console.log(`  Mode: ${mode} — found ${slideCount} ${mode === 'slides' ? 'slides' : 'sections'}`);

// Screenshot each slide / section
mkdirSync(SCREENSHOT_DIR, { recursive: true });
const screenshotPaths = [];

// Disable scroll-snap so we can position freely
await page.evaluate(() => {
...
// Force ALL .animate-in elements to .visible up front (one-time, before loop)
// github-guide's IntersectionObserver may have already fired for the first section;
// this guarantees all subsequent sections are also ready to screenshot.
await page.evaluate(() => {
...
for (let i = 0; i < slideCount; i++) {
  if (mode === 'slides') {
    // frontend-slides: show only the target slide
    ...
    // github-guide: scroll the section into view (all sections remain visible)
    await page.evaluate((index) => {
      const sections = document.querySelectorAll('main > section, body > section');
      const target = sections[index];
      if (!target) return;
      // Force-visible all animate-in descendants
      target.querySelectorAll('.animate-in').forEach(el => el.classList.add('visible'));
      // Also reset stagger delays so everything appears immediately
      target.querySelectorAll('.animate-in').forEach(el => {
        el.style.transitionDelay = '0s';
        el.style.opacity = '1';
        el.style.transform = 'none';
      });
      // Scroll without smooth behavior so screenshot timing is deterministic
      target.scrollIntoView({ behavior: 'instant', block: 'start' });
      window.scrollBy(0, 0);
    }, i);
  }

  // Wait for layout, fonts, and any nested animation (sparklines, radar polygons)
